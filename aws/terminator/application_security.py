...
class WafWebAcl(Waf):
    ...
    def terminate(self):
        if updates := [
            {'Action': 'DELETE', 'ActivatedRule': i}
            for i in self.client.get_web_acl(WebACLId=self.id)['WebACL']['Rules']
        ]:
...
class WafRule(Waf):
    ...
    def terminate(self):
        if updates := [
            {'Action': 'DELETE', 'Predicates': i}
            for i in self.client.get_rule(RuleId=self.id)['Rule']['Predicates']
        ]:
...
class WafXssMatchSet(Waf):
    ...
    def terminate(self):
        if updates := [
            {'Action': 'DELETE', 'XssMatchTuple': i}
            for i in self.client.get_xss_match_set(XssMatchSetId=self.id)[
                'XssMatchSet'
            ]['XssMatchTuples']
        ]:
...
class WafGeoMatchSet(Waf):
    ...
    def terminate(self):
        if updates := [
            {'Action': 'DELETE', 'GeoMatchConstraint': i}
            for i in self.client.get_geo_match_set(GeoMatchSetId=self.id)[
                'GeoMatchSet'
            ]['GeoMatchConstraints']
        ]:
...
class WafSqlInjectionMatchSet(Waf):
    ...
    def terminate(self):
        if updates := [
            {'Action': 'DELETE', 'SqlInjectionMatchTuple': i}
            for i in self.client.get_sql_injection_match_set(
                SqlInjectionMatchSetId=self.id
...
class WafIpSet(Waf):
    ...
    def terminate(self):
        if updates := [
            {'Action': 'DELETE', 'IPSetDescriptor': i}
            for i in self.client.get_ip_set(IPSetId=self.id)['IPSet'][
                'IPSetDescriptors'
            ]
        ]:
...
class WafSizeConstraintSet(Waf):
    ...
    def terminate(self):
        if updates := [
            {'Action': 'DELETE', 'SizeConstraint': i}
            for i in self.client.get_size_constraint_set(
                SizeConstraintSetId=self.id
...
class WafByteMatchSet(Waf):
    ...
    def terminate(self):
        if updates := [
            {'Action': 'DELETE', 'ByteMatchTuple': i}
            for i in self.client.get_byte_match_set(ByteMatchSetId=self.id)[
                'ByteMatchSet'
            ]['ByteMatchTuples']
        ]:
...
class WafRegexMatchSet(Waf):
    ...
    def terminate(self):
        if updates := [
            {'Action': 'DELETE', 'RegexMatchTuple': i}
            for i in self.client.get_regex_match_set(RegexMatchSetId=self.id)[
                'RegexMatchSet'
            ]['RegexMatchTuples']
        ]:
...
class WafRegexPatternSet(Waf):
    ...
    def terminate(self):
        if updates := [
            {'Action': 'DELETE', 'RegexPatternString': i}
            for i in self.client.get_regex_pattern_set(RegexPatternSetId=self.id)[
                'RegexPatternSet'
            ]['RegexPatternStrings']
        ]:
